...
|------|---------|---------|
| `feeds/feeds.example.txt` | Yes | Template — copy to `feeds/feeds.txt` and edit |
| `feeds/sitemap.example.txt` | Yes | Quick sitemap-only test |
| `feeds/feeds.txt` | No (gitignored) | Your personal route list |
| `feeds/.obf-failed.txt` | No (gitignored) | Auto-generated by `--source` — failed URLs |
